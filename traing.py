...
i = 0
slist = np.load('fin.npy')
with open('cftraingrade.json','r') as f:
    cf = json.load(f)
slidelist1 = []
for slide in slist:
    if slide in cf:
        ...
        i = i + 1
i = 0
with open('cftestgrade.json','r') as f:
    cf = json.load(f)
slidelist2 = []
for slide in slist:
    if slide in cf:
...
def train(args):
    ...
    if args.mission == 'envelop' or args.mission == 'necrosis':
        ...
        valdataset = WSIimageDataset(is_training = False,
                                        mission = args.mission)
        # num_workers
        valdataloader = DataLoader(traindataset,batch_size = 16)
        classnum = 2
        ...
        valdataset = ImageDataset(slidelist2,512,1,False)                            
        valdataloader = DataLoader(valdataset,batch_size = 32)
        classnum = 2
    ...
    checkpoints_dir = f'temp/checkpoints/{args.backbone}'
    results_dir = f'temp/results/{args.backbone}'
    ...
    for epoch in range(args.epoch):
        ...
        with torch.set_grad_enabled(True):
            for imgs,label1 in progress_bar:
                #i = i + 1
                #print(imgs.type)
                #print(label.type)

                imgs = imgs.float().cuda()
                label = label1.float().cuda()
        ...
        if bestacc < np.mean(epoch_acc_val):
            torch.save(model.state_dict(),f'temp/checkpoints/{args.backbone}/{args.backbone}_{epoch}_{args.mission}_1_best.pth.tar')
        ...
        model.eval()
        if (epoch+1)%5 == 0:
            torch.save(model.state_dict(),f'temp/checkpoints/{args.backbone}/{args.backbone}_{epoch}_{args.mission}_1.pth.tar')
        with open(f'temp/results/{args.backbone}/re_{args.mission}_{epoch}.pkl','wb') as f:
